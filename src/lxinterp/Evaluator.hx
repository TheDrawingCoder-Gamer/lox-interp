...
class Evaluator {
    ...
    public function new(exprs:Array<Expr>, ?fileName:String) {
        ...
        for (file in FileSystem.readDirectory(Lox.stdpath)) {
            if (file.endsWith(".lx") && file != Path.withoutDirectory(fileName)) {
                this.exprs.unshift(new Expr(EImport("<all>", 'std:' + file),
                    new Position(new Point(0, 0, 0), new Point(0, 0, 0), "<autogenerated import>")));
            }
        } 
    ...
    public function evalExpr(expr:Expr, isInline:Bool = true):Dynamic {
        switch (expr.type) {
            ...
            case ENil: return null;
            // TODO: EThis
            case EIdent(name):
                return lookup(name, expr);
            case EGroup(e): return evalExpr(e);
            ...
            case ECall(e, args): 
                ...
                var goodargs:Array<Dynamic> = args.map(function(e) { return evalExpr(e); });
            ...
            case EAssign(id, e): 
                ...
                } else globals.assign(id, v, expr);
                return v;
            case EArrayDecl(items): 
                return items.map(function(e) { return evalExpr(e); });
