...
class Lexer {
	...
	public function tokenize() {
		while (!isAtEnd()) {
			...
			switch (c) {
				...
				case "<":
					if (match("="))
						addToken(TLessEquals)
					else
						addToken(TLessThan);
				case ">":
					if (match("="))
						addToken(TGreaterEquals)
					else
						addToken(TGreaterThan);
				case "/":
					if (match("/")) {
						while (!isAtEnd() && peek() != '\n') {
							consume();
				...
				case '@':
					if (match('"'))
						...
						Lox.error(correctPosition(), "@ may only precede verbatim strings.");
				default:
					if (isDigit(c))
						number();
					else if (isAlpha(c)) {
	...
	private function match(char:String) {
		if (isAtEnd())
			return false;
		if (src.charAt(curPos.absPos) != char)
			return false;
		curPos.absPos++;
		curPos.column++;
		return true;
